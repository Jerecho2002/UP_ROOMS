...
const START_HOUR = 6;
const END_HOUR = 22; // For day view, ends after 10 PM
const HOUR_SLOTS = Array.from({ length: (END_HOUR - START_HOUR) * 2 }, (_, i) => {
    const hour = START_HOUR + Math.floor(i / 2);
    const minute = (i % 2) * 30;
    return { hour, minute, label: `${hour <= 12 ? hour : hour - 12}:${minute === 0 ? '00' : '30'} ${hour < 12 ? 'AM' : 'PM'}` };
});
...
// Extend Date prototype methods needed (must be done outside of the component or here)
...
const getEventsForDay = (date) => {
  return state.data.filter(event => {
    // Check if the event spans the given date
    const start = event.start.toDateString();
    const end = event.end ? event.end.toDateString() : start;
...
// MONTH VIEW: Generates the 6x7 grid of dates
const dateGrid = computed(() => {
  ...
  currentDate.setDate(currentDate.getDate() - startingDayIndex); // Start on Sunday

  for (let j = 0; j < 6; j++) { // Rows (max 6 weeks)
    const week = [];
    for (let i = 0; i < 7; i++) { // Days (7 columns)
    ...
    // Optimization: Stop if the first day of the next month has been reached and it's not the first row
    if (j > 0 && currentDate.getMonth() !== state.date.getMonth() && currentDate.getDate() === 1) break;
...
const weekDays = computed(() => {
  ...
  startOfWeek.setDate(state.date.getDate() - state.date.getDay()); // Go back to Sunday

  for (let i = 0; i < 7; i++) {
    const day = new Date(startOfWeek);
    day.setDate(startOfWeek.getDate() + i);
    
    // Get time-specific events for this day
    ...
    days.push({
      ...
      isToday: isToday(day),
      // Events are mapped to time slots later in the template
      events: dayEvents,
      allDayEvents: getEventsForDay(day).filter(e => e.allDay),
    });
  }
...
// DAY VIEW: Generates a single day's data
const singleDay = computed(() => {
  ...
  return {
    date: day,
    label: `${DAYS[day.getDay()]} ${day.getDate()}`,
    isToday: isToday(day),
...
function generateFakeData() {
  ...
  for(let i = 0; i < 50; i++) {
    ...
    // Fix: Ensure the date part of the end date doesn't slip too far
...
onMounted(() => {
  // Check the URL query parameters or route to set the initial mode if necessary
});
...
<template>
  <div class="p-4">
    <div class="flex flex-col md:flex-row justify-between items-center mb-4 space-y-3 md:space-y-0">
      ...
      <div class="flex space-x-2">
        <div class="flex space-x-1">
          <button @click="changeDate(-1, state.mode)" class="p-2 border rounded-md text-gray-700 hover:bg-gray-100">&lt;</button>
          <button @click="changeDate(1, state.mode)" class="p-2 border rounded-md text-gray-700 hover:bg-gray-100">&gt;</button>
        </div>
        <button @click="goToToday" :class="['p-2 border rounded-md text-sm transition', isToday(state.date) ? 'bg-red-600 text-white' : 'text-gray-700 hover:bg-gray-100']">
          Today
        </button>
      </div>
          ...
          <tr v-for="(week, index) in dateGrid" :key="index">
            <td v-for="day in week" :key="day.date.toDateString()" 
                :class="[
                  ...
                  day.dayClass, // Text color for outside dates
                  day.isToday ? 'bg-yellow-100 ring-2 ring-yellow-500/50' : 'hover:bg-blue-50',
                  day.date.toDateString() === state.date.toDateString() ? 'bg-blue-100' : ''
                ]">
            ...
            <td v-for="day in weekDays" :key="day.date.toDateString() + slot.label" 
              ...
              <template v-for="event in day.events" :key="event.id">
                <div v-if="event.start.getHours() === slot.hour && event.start.getMinutes() === slot.minute"
                     ...
                     :style="{ height: (event.end ? (event.end.getHours() * 60 + event.end.getMinutes() - event.start.getHours() * 60 - event.start.getMinutes()) / 30 * 2.5 : 2.5) + 'rem' }"
    ...
    <div v-else class="text-center py-12 text-gray-500">
      <p>Viewing mode: **{{ state.mode.toUpperCase() }}**.</p>
      <p>The **Year** view is not fully implemented.</p>
    </div>
  </div>
</template>