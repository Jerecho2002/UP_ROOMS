                ...
                <div class="space-y-4 mt-4">
                    <div class="flex justify-between items-center pt-4">
                        <div class="relative w-full max-w-sm">
                            ...
                            />
                            <FontAwesomeIcon :icon="icons.search" class="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
                        </div>
                    ...
                    <div class="bg-white p-6 rounded-xl shadow-xl overflow-x-auto">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <select v-model="itemsPerPage" @change="handlePerPageChange" class="text-sm border border-gray-300 rounded px-2 py-1">
                        ...
                        <table class="min-w-full divide-y divide-yellow-600">
                            ...
                            <tbody class="bg-white divide-y divide-yellow-600">
                                ...
                                <tr v-for="room in roomList" :key="room.id" class="hover:bg-gray-200 transition duration-100">
                                    <td class="px-6 py-4 text-sm font-mono text-gray-500 cursor-pointer" @click="selectRoomForSidebar(room)">{{ room.id }}</td>
                                    <td class="px-6 py-4 text-sm font-medium text-gray-900 cursor-pointer" @click="selectRoomForSidebar(room)">
                                        {{ room.room_name }}
                                        <span class="text-xs text-gray-500 block">{{ room.room_code }}</span>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-800 cursor-pointer" @click="selectRoomForSidebar(room)">
                                        {{ room.building?.building_name || 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-800 cursor-pointer" @click="selectRoomForSidebar(room)">
                                        {{ room.college?.college_name || 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-800 text-center font-bold cursor-pointer" @click="selectRoomForSidebar(room)">
                                        {{ room.capacity }}
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-800 cursor-pointer" @click="selectRoomForSidebar(room)">
                                        {{ room.location || 'N/A' }}
                                    </td>
                                    <td class="px-6 py-4 text-sm cursor-pointer" @click="selectRoomForSidebar(room)">
                                        <span :class="{
                        ...
                        <!-- Pagination -->
                        <div v-if="pagination.last_page > 1" class="flex flex-col sm:flex-row items-center justify-between pt-4 border-t border-gray-200 mt-4">
