...
defineProps({
    ...
    // Removed tableData prop as we rely on Inertia's 'rooms' prop
});
...
const handleViewDetails = (room) => {
    // Safely check for the first schedule entry
    const schedule = room.schedules && room.schedules.length > 0 ? room.schedules[0] : {};
    ...
    currentViewedDetails.value = {
        'User Name': room.user_account?.username || 'N/A',
        'College': room.college?.college_name || 'N/A',
        'Location': room.location || 'N/A',
        'Room Name': room.room_name || 'N/A',
         'CFIC ID': room.schedules?.cfic_id || 'N/A',
        'Start Date': formatDate(schedule.start_time),
        'End Date': formatDate(schedule.end_time),
    };
...
<template>
    <div class="relative min-h-screen">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                ...
                                >
                                    ...
                                    <td class="px-4 py-3 space-x-2">
                                        <button @click="handleViewDetails(room)" class="text-green-500 hover:text-green-700" title="View">👁️</button>
        ...
        <transition name="fade">
            <div v-if="isDetailsModalVisible" class="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
                <div class="bg-white rounded-lg shadow-2xl w-full max-w-lg p-6 relative">
                    ...
                    <div v-if="currentViewedDetails" class="space-y-3">
                        <div v-for="(value, key) in currentViewedDetails" :key="key" class="flex justify-between items-center text-gray-700">
